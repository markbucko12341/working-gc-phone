  ...
  if PhoneNumbers[type] ~= nil then
    ...
    for key, alert in pairs(PhoneNumbers[type].alerts) do
      ...
      if alert.id == alertId then
        ...
        TriggerClientEvent('esx_addons_gcphone:showMessage', tonumber(key), 'Units from ' .. PhoneNumbers[type].type .. ' in route.')
      ...
      if alert.id == alertId then
        ...
        if allRefuse == true then
          if PhoneNumbers[type].alerts[key].isAccept == 0 then
            TriggerClientEvent('esx_addons_gcphone:showMessage', tonumber(key), 'All units ignored calls')
            for k, src in pairs(PhoneNumbers[type].alerts[key].sources) do 
              TriggerClientEvent('esx_addons_gcphone:showMessage', tonumber(k), 'All units ignored calls')
            end
          end
